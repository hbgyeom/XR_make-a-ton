import sys
import subprocess
#py파일 안에서 라이브러리를 설치하기 위해 subprocess로 pip 실행
subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'numpy==1.26.4'])
subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'matplotlib==3.9.2'])
subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'praat-parselmouth==0.4.5'])
subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'SpeechRecognition==3.11.0'])
subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'huggingface-hub==0.26.2'])
subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'gTTS==2.5.3'])
subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'faster-whisper==1.0.3'])
import numpy as np
...
#출력 시 buffer에 저장되면 c#에서 redirection이 안되기 때문에 print후에 flush
...
def transcribe_audio(audio_queue, plot_queue):
    ...
    while True:
        ...
        try:
            ...
            print("Error during transcription:", e, flush=True)
...
def plot_graph(plot_queue):
    ...
    while True:
        ...
        if text == "":
            print("Text is empty, skipping plot.", flush=True)
            continue
        elif text == ".":
            print("Text is just a period, skipping plot.", flush=True)
            continue
        ...
        print("plt show", flush=True)
        plt.tight_layout()
        # 그래프를 보여주고 3초 후에 자동으로 닫기
        plt.draw()
        plt.pause(3)  # 3초 동안 그래프 표시
        plt.close()   # 그래프 창 닫기 
        print("plt show after", flush=True)
